...
def motor_right_reverse(speed):
    L_Motor.ChangeDutyCycle(0)
    GPIO.output(AIN2, False)  # 이 부분을 수정
    GPIO.output(AIN1, True)   # 이 부분을 수정
    R_Motor.ChangeDutyCycle(speed)
    GPIO.output(BIN2, True)   # 이 부분을 수정
    GPIO.output(BIN1, False)  # 이 부분을 수정

def motor_left_reverse(speed):
    L_Motor.ChangeDutyCycle(speed)
    GPIO.output(AIN2, True)
    GPIO.output(AIN1, False)
    R_Motor.ChangeDutyCycle(0)
...
def stopline_img_preprocess(image):
    ...
    image = cv2.GaussianBlur(image, (5, 5), 0)
    _, image = cv2.threshold(image,200, 255, cv2.THRESH_BINARY_INV)        # 임계값 140이었음, 180이 일단 베스트
...
def process_frames():
    ...
    try:
        while True:
            ...
            if carState == "go":
                if 70 <= steering_angle <= 95:     # 70, 100이었음
                    print("go")
                    speedSet = 45
                    motor_go(speedSet)
                    ...
                    motor_right_reverse(10)  # 모터를 중간 속도로 뒤로 회전시킴
                elif steering_angle < 70:
                    print("left")
                    speedSet = 40   #35
                    motor_left(speedSet)
                    motor_left_reverse(10)  # 모터를 중간 속도로 뒤로 회전시킴
